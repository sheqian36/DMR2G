...
from src.modeling.modeling_bart import BartModel_iu_xray
from src.modeling.modeling_bart import BartModel_mimic_cxr
import torch
import torch as th
...
class TransformerNetModel_encoder_decoder(nn.Module):
    ...
    def __init__(
        ...
        self.img_cls_head = nn.Sequential(nn.Linear(512, 512), nn.Linear(512, 14))
        self.txt_cls_head = nn.Sequential(nn.Linear(512, 512), nn.Linear(512, 14))
    ...
    def build_xstart_predictor(self):
        if self.init_pretrained:
            ...
            if self.task == 'iu_xray':
                self.input_transformers = BartModel_iu_xray(self.config, self.embedding_dim)
            else:
                self.input_transformers = BartModel_mimic_cxr(self.config, self.embedding_dim)
    ...
    def forward_encoder(self, 
        ...
        # img_embeds, image_embedding = self.encode_image(input_embed)
    ...
    def forward(self, 
        ...
        output = self.input_transformers(
            input_ids = None,
            attention_mask=attention_mask,
            input_embeds = input_embed,
            decoder_input_ids=None,
        ...
        img_output = self.img_cls_head(th.mean(output['encoder_last_hidden_state'], dim=1))
        txt_output = self.txt_cls_head(th.mean(output['last_hidden_state'], dim=1))


        h = self.output_down_proj(input_trans_hidden_states)

        return h, img_output, txt_output
