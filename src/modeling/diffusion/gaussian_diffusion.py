...
import logging
logging.basicConfig(filename=f"model_output.txt", level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.bce_loss = th.nn.BCEWithLogitsLoss()
    ...
    def _loss_history_update(self, ts, losses, loss_masks, training_step): #v5
        ...
        if training_step % self._loss_history_update_stride == 0:
    ...
    def training_losses(self, model, training_step, t, model_kwargs=None, noise=None):
        ...
        label = model_kwargs.pop("label").to(t.device)
        ...
        if np.random.uniform() > 0.5:
            with th.no_grad():
                model_output, img_output, txt_output = model(x = x_t, ts = self._scale_timesteps(t), **model_kwargs)
        ...
        if training_step % 100 ==0:
            ...
            cands = torch.topk(logits, k=1, dim=-1).indices.squeeze()
            decoded_sentences = []
            for seq in cands:
                # seq = seq[seq>2]
                decoded_sentence = self.tokenizer.decode(seq.tolist(), skip_special_tokens=False)
                # decoded_sentence = tokenizer.decode(seq.tolist(), skip_special_tokens=False)
                decoded_sentences.append(decoded_sentence)
            logging.info(f'training_step: {training_step}')
            logging.info(decoded_sentences[-1])


        decoder_nll = self.token_discrete_loss(x_start, get_logits, input_ids, training_step=training_step, mask=loss_mask)
                   
        img_bce_ls = self.bce_loss(img_output, label)
        txt_bce_ls = self.bce_loss(txt_output, label)
        
        #     terms["loss"] = terms["mse"] + (decoder_nll + tT_loss)
        # terms["loss"] = terms["mse"] + tT_loss
        terms["loss"] = 2 * terms["mse"] + (decoder_nll + tT_loss) + (img_bce_ls + txt_bce_ls)
    ...
    def token_discrete_loss(self, x_t, get_logits, input_ids, training_step, mask=None):
        ...
        # if training_step > 50: decoder_nll = th.zeros_like(decoder_nll, dtype=th.float32)
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        model_output, img_output, txt_output = model(x, self._scale_timesteps(t), **model_kwargs)
