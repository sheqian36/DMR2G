...
VIT_TYPE = '/home/shuchenweng/cz/oyh/model/seqdiffuseq/swin-tiny'
...
class MedCLIPVisionModelViT(nn.Module):
    ...
    def __init__(self, checkpoint=None, medclip_checkpoint=None) -> None:
        ...
        self.model = AutoModel.from_pretrained(self.vit_type, output_hidden_states=True, output_attentions=True)
    ...
    def forward(self, pixel_values, project=True):
        ...
        if project:
            hidden_states = self.projection_head(hidden_states)
            # encoder_states = self.projection_head(encoder_states)
            # attentions = self.projection_head(attentions)
        #     image_embedding = self.projection_head(image_embedding)
        # last_hidden_state:  torch.Size([32, 49, 512])
        return hidden_states, encoder_states, attentions
        # return img_embeds, image_embedding
...
class MedCLIPModel(nn.Module):
    def __init__(self,
        ...
        assert vision_cls in [MedCLIPVisionModelViT], 'vision_cls should be one of [MedCLIPVisionModel, MedCLIPVisionModelViT]'
    ...
    def from_pretrained(self, input_dir=None):
        ...
        if isinstance(self.vision_model, MedCLIPVisionModelViT):
            ...
            if input_dir is None:
                input_dir = '/home/shuchenweng/cz/oyh/model/seqdiffuseq/pretrained/medclip-vit'
        else:
    ...
    def encode_image(self, pixel_values=None):
        # image encoder
        # vision_output = self.vision_model(pixel_values=pixel_values)encoder_outputs
        # img_embeds = vision_output / vision_output.norm(dim=-1, keepdim=True)
        # return img_embeds
        hidden_states, encoder_states, attentions = self.vision_model(pixel_values=pixel_values)
        return hidden_states, encoder_states, attentions
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        hidden_states, encoder_states, attentions = self.encode_image(pixel_values)
        return hidden_states, encoder_states, attentions
...
class VisualExtractor(nn.Module):
    ...
    def forward(self, images):
        ...
        patch_feats = self.projection_head(patch_feats)
        
        # return BaseModelOutput(
        #     last_hidden_state=patch_feats, hidden_states=None, attentions=None
        # )
        print("patch_feats shape ", patch_feats.shape)
...
def build_transformer():  # detr decoder
    return Transformer(
        d_model=512, #args.hidden_dim,
        dropout=0.1, #args.dropout,
        nhead=8, #args.nheads,
        dim_feedforward=2048, #args.dim_feedforward,
        num_encoder_layers=6, #args.enc_layers,
        num_decoder_layers=6, #args.dec_layers,
        normalize_before=True, #args.pre_norm, store_true
...
class Transformer(nn.Module):
    ...
    def forward(self, 
        ...
        return hs.transpose(1, 2).squeeze(0)
class DETR(nn.Module):
    ...
    def forward(self, 
        ...
        hs = self.transformer(input_embed, mask, self.query_embed.weight)

        # return torch.squeeze(hs, dim=1)
        return hs
...
from graph.tagencoder import TagEncoder
from graph.tagencoder import *

class BartModel_iu_xray(BartPretrainedModel):
    def __init__(self, config: BartConfig, embedding_dim: int):
        ...
        self.encoder = MedCLIPModel(vision_cls=MedCLIPVisionModelViT)
        self.encoder.from_pretrained()

        # resnet-101
        # self.backbone = VisualExtractor()

        # graph
        self.tag_encoder = TagEncoder(0.1)
        c = copy.deepcopy
    ...
    def graph_image(self, image_feat, knowledge_skg):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            # medclip
            fc_feats_0, _, _ = self.encoder(pixel_values = input_embeds[:, 0])
            fc_feats_1, _, _ = self.encoder(pixel_values = input_embeds[:, 1])
            patch_feats = torch.cat((fc_feats_0, fc_feats_1), dim=1)
            patch_feats = self.graph_image(patch_feats, knowledge_skg)

            encoder_outputs = BaseModelOutput(last_hidden_state=patch_feats, hidden_states=None, attentions=None)
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_hidden_states=encoder_outputs[0], # ? B N(H*W) D  torch.Size([32, 49, 512])
...
class BartModel_mimic_cxr(BartPretrainedModel):
    def __init__(self, config: BartConfig, embedding_dim: int):
        super().__init__(config)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            # medclip
            fc_feats, _, _ = self.encoder(pixel_values = input_embeds)
            encoder_outputs = BaseModelOutput(last_hidden_state=fc_feats, hidden_states=None, attentions=None)
