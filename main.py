...
def main():
    ...
    # # print()tokenizer.tokenize("normal <0> cardiomegaly <0> scoliosis <0> fractures <0> effusion <0> thickening <0> pneumothorax <0> emphysema <0> pneumonia <0> edema <0> hernia <1> calcinosis <0> medical device <0> hypoinflation <0> airspace disease")
    ...
    # args.vocab_size = tokenizer.vocab_size + 2
    
    logger.log("creating model and diffusion...", args.checkpoint_path)
    model, diffusion = create_model_and_diffusion(
        pad_tok_id=tokenizer.pad_token_id if hasattr(tokenizer, 'pad_token_id') else tokenizer.get_vocab()['<pad>'],  # tokenizer.token2idx['<pad>'],
