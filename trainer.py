...
class Trainer:
    ...
    def forward_only(self, batch, cond):
        ...
        batch_size = batch['input_embed'].shape[0]
        with torch.no_grad():
            zero_grad(self.model_params)
            for i in range(0, batch_size, self.microbatch):
                micro = {key: item[i : i + self.microbatch].to(dist_util.dev()) for key, item in batch.items() if (key != 'input_ids' and key != 'image_id')} # and key != 'label'
                if cond == None:
                    micro_cond = None
                else:
                    micro_cond = {
                ...
                t, weights = self.schedule_sampler.sample(micro['input_embed'].shape[0], dist_util.dev())
    ...
    def _log_grad_norm(self):
        sqsum = 0.0
        # print("master_params: ", self.master_params)
        ...
        #     if param.grad == None or 'input_transformers.encoder.text_model' in name:
    ...
    def log_step(self):
        ...
        # print("all steps: ", self.step + self.resume_step)
