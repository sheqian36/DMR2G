...
from src.modeling.model_utils import create_model_and_diffusion
...
# from tokenizer_pre import Tokenizer
...
def main():
    ...
    training_args['test_txt_path'] = args.test_txt_path
    ...
    training_args['dataset'] = args.dataset
    ...
    tokenizer = create_tokenizer(return_pretokenized=args.use_pretrained_tokenizer,
                                 path=f"/home/shuchenweng/cz/oyh/data/seqdiffuseq/{args.dataset}",
                                 tokenizer_type='byte-level',
                                 tokenizer_ckpt=args.pretrained_tokenizer)
    # tokenizer.add_special_tokens(['<0>', '<1>'])
    # print("<1>: ", tokenizer.encode('<1>').ids)
    # print("<1>: ", tokenizer.decode((tokenizer.encode('<1>').ids), skip_special_tokens=False))
    # tokenizer = Tokenizer(args)
    
    model, diffusion = create_model_and_diffusion(
        pad_tok_id=tokenizer.pad_token_id if hasattr(tokenizer, 'pad_token_id') else tokenizer.get_vocab()['<pad>'],
        resume_checkpoint=args.resume_checkpoint, task=args.task, tokenizer=tokenizer, **args_to_dict(args, model_and_diffusion_defaults().keys())
    )
    ...
    if args.num_samples <= 0:
        args.num_samples = len(dataloader_utils.TextDataset_translation(tokenizer=tokenizer, data_path=args.test_txt_path,
                                                                        label_path=args.label_path, # source=args.src, target=args.tgt,
                                                                       ))
    ...
    while len(all_samples) * args.batch_size < args.num_samples:
        ...
        model_kwargs = {key:item.to(dist_util.dev()) for key, item in batch.items() if ('decoder' not in key and key != 'input_ids' and key != 'image_id' and key != 'label')}
    ...
    for seq in cands:
        ...
        # decoded_sentence = tokenizer.decode(seq.tolist(), skip_special_tokens=False)
    ...
    for seq in ground_true_samples:
        ...
        # ground_true_sentence = tokenizer.decode(seq.squeeze().tolist(), skip_special_tokens=False)
        ground_true_sentences.append(ground_true_sentence)

    # log = dict()
    # test_met = compute_scores({i: [gt] for i, gt in enumerate(ground_true_sentences)},
    #                             {i: [re] for i, re in enumerate(decoded_sentences)})
    ...
    #     gts[image_ids[i]] = ground_true_sentences[i]
    #     res[image_ids[i]] = decoded_sentences[i]
    # json.dump(gts, open(os.path.join(model_dir, f"{model_base_name}_gts.json"), "w"))
    # json.dump(res, open(os.path.join(model_dir, f"{model_base_name}_res.json"), "w"))
