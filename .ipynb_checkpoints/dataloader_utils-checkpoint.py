...
#         # self.ann = pd.read_csv(self.ann_path, sep='\t', names=['image_path', 'report'])
...
class TextDataset_translation(TextDataset):
    ...
    def read_data(self):
        ...
        # dataset = pd.read_csv(self.data_path, sep="\t", names=['img', 'sent'])
        ...
        #     txt = dataset['sent'][i]
        #     src = txt[:int(len(txt)/2)]
        #     tgt = txt[int(len(txt)/2):]
        #     src_text.append(src)
        #     tgt_text.append(tgt)
